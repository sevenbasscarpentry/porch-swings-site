...
export async function handler() {
  ...
  if (!token || !siteId) {
    return { statusCode: 500, headers, body: JSON.stringify({ error: 'Missing FORMS_ACCESS_TOKEN or FORMS_SITE_ID environment variable(s).', hasToken: !!token, hasSiteId: !!siteId }) };
  }
  try {
    ...
    const rows = subs.map(s => ({ id:s.id, created_at:s.created_at, name:s.data?.name||'', email:s.data?.email||'', phone:s.data?.phone||'', city:s.data?.city||'', message:s.data?.message||'' })).sort((a,b)=> new Date(b.created_at) - new Date(a.created_at));
